...
TEST(onionpir, onionpir_trace_expand) {
  ...
  uint64_t b_size = 7;
  uint64_t rlwe_size = a_size + b_size;
  uint64_t res_nrows = 13;
  uint64_t res_ncols = 16;

  // output is scaled by 2^#steps
  const uint64_t res_mult_coef = 1ul << (uint64_t)ceil(log2(res_nrows));

  // generate some random plaintext in the first res_rlwe_nrows positions
  std::vector<int64_t> message(mu_size * ONIONPIR_N, 0);
  for (uint64_t i = 0; i < mu_size; ++i) {
    random_centered_reduced(res_nrows, ONIONPIR_K, message.data() + i * ONIONPIR_N);
  }
  ...
  for (uint64_t i = 0; i < res_nrows; ++i) {
    message_and_noise decr =
        decrypt(module.get(), mu_size * ONIONPIR_K, skey, res_rlwes.data() + i * res_ncols * ONIONPIR_N, res_ncols);
    ASSERT_GT(decr.noise_amplitude, 0.);  // noiseless?
    ASSERT_LE(decr.noise_amplitude, pow(2., -96));
    std::cerr << "output noise: " << log2(decr.noise_amplitude) << std::endl;
    ...
    // result const coefficient should be the i-th input coefficient
    int64_t ci = 0;
    for (int64_t k = mu_size - 1; k >= 0; --k) {
      const auto exp_co = centermod_and_carry(ci + message[k * ONIONPIR_N + i] * res_mult_coef);
      ci = exp_co.second;
      int64_t act = decr.decrypted[k * ONIONPIR_N];
      ASSERT_EQ(act, exp_co.first);
    }

    // other coefficients are zero
    for (uint64_t k = 0; k < mu_size; ++k) {
      for (uint64_t j = 1; j < ONIONPIR_N; ++j) {
        ASSERT_EQ(decr.decrypted[k * ONIONPIR_N + j], 0);
      }
    }
  }
}
